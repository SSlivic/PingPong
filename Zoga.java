...
public void premakni() {
	...
	this.x+=xPremik;
	this.y+=yPremik;//to spu�ta pod 45 stepeni ugao
}
private Rectangle getMejeZoge() {
	return new Rectangle(this.x,this.y,PREMER,PREMER);
}
private boolean trkZLoparjem() {
	return igra.lopar.getMejeLoparja().intersects(this.getMejeZoge());//intersekt povrne boolean da li se seku ili ne.da li su  u istoj povr�ini
}
private boolean trkZLoparjem2() {
	return igra.lopar2.getMejeLoparja2().intersects(this.getMejeZoge());//intersekt povrne boolean da li se seku ili ne.da li su  u istoj povr�ini
	}
